...
MinValueIndex findMinValueAndIndex(double grid[MAP_ROW][MAP_COL]) {
...
void qr_code_callback(const char* qr_code_data) {
    ...
    if (strlen(qr_code_data) == 2) {
        ...
        Action action = leave_trap(row, col) ? setBomb : move;  // 폭탄 설치
...
int main(int argc, char *argv[]) {
    ...
    // mutex 초기화
    if (pthread_mutex_init(&bomb_mutex, NULL) != 0) {

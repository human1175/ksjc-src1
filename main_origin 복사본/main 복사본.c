...
DGIST dgist;  // 전역 변수로 설정

int player_number = 0;
int opponent_number = 1;
...
node_c find_best_node(int current_row, int current_col) {
    int min_value = 0;
    ...
    //     {current_row + 1, current_col}, // 하
    //     {current_row, current_col - 1}, // 좌
    //     {current_row, current_col + 1}  // 우
    // };

    int up_row = current_row - 1;
    int up_col = current_col;
    int down_row = current_row + 1;
    int down_col = current_col;
    int right_row = current_row;
    int right_col = current_col + 1;
    int left_row = current_row;
    int left_col = current_col - 1;

    // 유효한 좌표인지 확인
    if (up_row >= 0 && up_row < 5 && up_col >= 0 && up_col < 5) {
        // up
        Item item = dgist.map[up_row][up_col].item;
        int value = 0;
        if (item.status == 1) {
            value = item.score;
        ...
        if (value < min_value) {
            min_value = value;
            best_node = {up_row, up_col, value};
        }
    }
    if (down_row >= 0 && down_row < 5 && down_col >= 0 && down_col < 5) {
        // down
        Item item = dgist.map[down_row][down_col].item;
        int value = 0;
        if (item.status == 1) {
            value = item.score;
        } else if (item.status == trap) {
        ...
        if (value < min_value) {
            min_value = value;
            best_node = {down_row, down_col, value};
        }
    }
    if (left_row >= 0 && left_row < 5 && left_col >= 0 && left_col < 5) {
        // left
        Item item = dgist.map[left_row][left_col].item;
        int value = 0;
        if (item.status == 1) {
            value = item.score;
        } else if (item.status == trap) {
        ...
        if (value < min_value) {
            min_value = value;
            best_node = {left_row, left_col, value};
        }
    }
    if (right_row >= 0 && right_row < 5 && right_col >= 0 && right_col < 5) {
        // right
        Item item = dgist.map[right_row][right_col].item;
        int value = 0;
        if (item.status == 1) {
            value = item.score;
        } else if (item.status == trap) {
        ...
        if (value < min_value) {
            min_value = value;
            best_node = {right_row, right_col, value};
        }
    }
...
int execute_strategy() {
    ...
    node_c best_node = find_best_node(cur_row, cur_col);
    int destination_node_row = best_node.row;
    int destination_node_col = best_node.col;
    printf("===============best_row : %d, best_col : %d===============\n", destination_node_row, destination_node_col);
    // printf("===============pre_row : %d, pre_col : %d===============\n", pre_row, pre_col);
    ...
    int row_diff = destination_node_row - cur_row;
    int col_diff = destination_node_col - cur_col;
...
int leave_trap(int row_dst, int col_dst) {
    ...
    client_info opponent = dgist.players[opponent_number];
...
void qr_code_callback(const char* qr_code_data) {
    ...
    if (strlen(qr_code_data) == 2) {
        ...
        int leaveTrap = leave_trap(row, col);
        Action action = move;
        int rt = rand()%6;
        if (leaveTrap == 1){
            action = setBomb;
